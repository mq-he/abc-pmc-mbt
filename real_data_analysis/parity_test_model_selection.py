"""
ABC model selection between the reducible (model 1) and
irreducible model (model 2), both processes starts in phase 1

Note:
the tolerance value is only updated when accept_rate >= 0.03
"""
...
def search(item):
    ...
    0: oviparous
    1: viviparous
    -1: no information or neither 0 or 1
    """
    if newname.get(item):
...
def NodeDeleteInfo(root):
    """Remove lineages where the information about parity node is missing
    """
...
while len(accept1)<M:
    ...
    indexmod = np.random.binomial(1,0.5) # 0: q12=0, 5 parameters; 1: q12!=0, 6 parameters
    ...
    if indexmod == 0:
        ...
        while do:
            ...
            w = dom([birth1sim, birth2sim], [death1sim, death2sim], [q12sim,0])  
            while w<=0.001:
                birth1sim = random.uniform(0,0.2)
                birth2sim = random.uniform(0,0.2)
                death1sim = random.uniform(0,0.2)
            ...
            while flag == 0:
                startphase=1
                t, flag = birth_death_tree2([birth1sim, birth2sim], [death1sim, death2sim], [q12sim,0], start=startphase, nsize=sizeobs[0])
                r+=1
        ...
        if min_tspan <= mtspan <= max_tspan and min_dist<=mdist<=max_dist and min_imb<=mbal<=max_imb and min_imb1<=mbal1<=max_imb1 and min_imb2<=mbal2<=max_imb2 and min_stat<=mstat<=max_stat and min_stat2<=mstat2<=max_stat2 and min_prop<=mprop<=max_prop:
            ...
            accept6.append(0)
            acceptmod.append(indexmod)

            domtemp.append(dom([birth1sim,birth2sim],[death1sim,death2sim],[q12sim,0]))
...
rate=[accept_rate]
mod2prop = [mean(acceptmod)] # the proportion of model 2 is selected
...
print('\n\n'+'accept1='+str(array1)+'\n\n'+'accept2='+str(array2)+'\n\n'+'accept3='+str(array3)+'\n\n'+'accept4='+str(array4)+'\n\n'+'accept5='+str(array5)+'\n\n'+'accept6='+str(array6))
print('\n\n'+'acceptmod='+str(acceptmod))
print('\n\n'+'mod2prop='+str(mod2prop))
...
def model_index(acceptmod):
    """
    Record the indices of the accepted samples in model 1 and model 2.
    Return two lists of indices
    """
    index_mod1, index_mod2 = [], []
    for i,mod in enumerate(acceptmod):
        if mod == 0:
            index_mod1.append(i)
        else:
            index_mod2.append(i)
...
def generate_tol(accept_mod1, accept_mod2, weight_mod1,weight_mod2, nsim=10, multiplier=40):
    if len(weight_mod1) == 0: # model 1 disappears, all samples are from model 2 (6 parameters)
        birth1=weighted_mean(accept_mod2[0,:], weight_mod2)
        birth2=weighted_mean(accept_mod2[1,:], weight_mod2)
        death1=weighted_mean(accept_mod2[2,:], weight_mod2)
        death2=weighted_mean(accept_mod2[3,:], weight_mod2)
        q12=weighted_mean(accept_mod2[4,:], weight_mod2)
        q21=weighted_mean(accept_mod2[5,:], weight_mod2)
        ...
        tol_dist = multiplier*sqrt(variance(distanceobs))
        tol_imb = multiplier*sqrt(variance(balobs))
        tol_imb1 = multiplier*sqrt(variance(bal1obs))
        tol_imb2 = multiplier*sqrt(variance(bal2obs))
        tol_tspan = multiplier*sqrt(variance(tspanobs))
        tol_prop = multiplier*sqrt(variance(propobs))
        tol_stat = multiplier*sqrt(variance(statobs))
        tol_stat2 = multiplier*sqrt(variance(stat2obs))

    elif len(weight_mod2) == 0: # model 2 disappears, all samples are from model 1 (6 parameters)
        birth1=weighted_mean(accept_mod1[0,:], weight_mod1)
        birth2=weighted_mean(accept_mod1[1,:], weight_mod1)
        death1=weighted_mean(accept_mod1[2,:], weight_mod1)
        death2=weighted_mean(accept_mod1[3,:], weight_mod1)
        q12=weighted_mean(accept_mod1[4,:], weight_mod1)
        q21=0
        ...
        tol_dist_mod1 = multiplier*sqrt(variance(distanceobs))
        tol_imb_mod1 = multiplier*sqrt(variance(balobs))
        tol_imb1_mod1 = multiplier*sqrt(variance(bal1obs))
        tol_imb2_mod1 = multiplier*sqrt(variance(bal2obs))
        tol_tspan_mod1 = multiplier*sqrt(variance(tspanobs))
        tol_prop_mod1 = multiplier*sqrt(variance(propobs))
        tol_stat_mod1 = multiplier*sqrt(variance(statobs))
        tol_stat2_mod1 = multiplier*sqrt(variance(stat2obs))
        ...
        tol_dist_mod2 = multiplier*sqrt(variance(distanceobs))
        tol_imb_mod2 = multiplier*sqrt(variance(balobs))
        tol_imb1_mod2 = multiplier*sqrt(variance(bal1obs))
        tol_imb2_mod2 = multiplier*sqrt(variance(bal2obs))
        tol_tspan_mod2 = multiplier*sqrt(variance(tspanobs))
        tol_prop_mod2 = multiplier*sqrt(variance(propobs))
        tol_stat_mod2 = multiplier*sqrt(variance(statobs))
        tol_stat2_mod2 = multiplier*sqrt(variance(stat2obs))

        tol_dist = max(tol_dist_mod1, tol_dist_mod2)
        tol_imb = max(tol_imb_mod1,tol_imb_mod2)
        tol_imb1 = max(tol_imb1_mod1, tol_imb1_mod2)
        tol_imb2 = max(tol_imb2_mod1, tol_imb2_mod2)
        tol_tspan = max(tol_tspan_mod1, tol_tspan_mod2)
        tol_prop = max(tol_prop_mod1, tol_prop_mod2)
        tol_stat = max(tol_stat_mod1, tol_stat_mod2)
        tol_stat2 = max(tol_stat2_mod1, tol_stat2_mod2)

    return tol_dist,tol_imb,tol_imb1,tol_imb2,tol_tspan,tol_prop,tol_stat,tol_stat2
...
for k in range(1,3):
    ...
    # for each iteration, find out the accepted samples in model 1
    index_mod1, index_mod2 = model_index(acceptmod)
    accept_mod1, accept_mod2 = accept[0:5,index_mod1], accept[:,index_mod2] # note that model 1 only has 5 parameters
    weight = np.array(weight)
    weight_mod1, weight_mod2 = weight[index_mod1], weight[index_mod2]
    if len(index_mod1) == 0:
        covmat2 = 2*np.cov(accept_mod2, aweights=weight_mod2) # model 1 disappears
    elif len(index_mod2) == 0:
        covmat1 = 2*np.cov(accept_mod1, aweights=weight_mod1) # model 2 disappears
    else:
        covmat1 = 2*np.cov(accept_mod1, aweights=weight_mod1)
        covmat2 = 2*np.cov(accept_mod2, aweights=weight_mod2)
    ...
    while len(accept1)<M:
        ...
        if len(index_mod1) == 0:
            indexmod = 1 # model 1 disappears
        elif len(index_mod2) == 0:
            indexmod = 0 # model 2 disappears
        else:
            # model selection step
            indexmod = np.random.binomial(1,0.5) # 0: q12=0, 5 parameters; 1: q12!=0, 6 parameters
        ...
        if indexmod==0:
            ...
            while do:
                do = False
                flag = 0
                ind = random.choices(index_mod1, weights=weight_mod1, k=1)[0]
                [birth1sim,birth2sim,death1sim,death2sim,q12sim]=accept.T[ind,0:5]+multivariate_normal.rvs(np.zeros(5),covmat1)
                w = dom([birth1sim, birth2sim], [death1sim, death2sim], [q12sim,0])  
                while w<=0.001 or min(birth1sim, birth2sim,death1sim,death2sim,q12sim)<=0.0 or max(birth1sim, birth2sim,death1sim,death2sim,q12sim)>=0.2:
                    ind = random.choices(index_mod1, weights=weight_mod1, k=1)[0]
                    [birth1sim,birth2sim,death1sim,death2sim,q12sim]=accept.T[ind,0:5]+multivariate_normal.rvs(np.zeros(5),covmat1)
                    w = dom([birth1sim, birth2sim], [death1sim, death2sim], [q12sim,0]) 
                r=0
            ...
            for j in range(1,len(sizeobs)):
                ...
                while flag == 0:
                    startphase=1
                    tsim, flag = birth_death_tree2([birth1sim, birth2sim], [death1sim, death2sim], [q12sim,0], start=startphase, nsize=sizeobs[j])
                assert flag == 1
                b,b1,b2=balance(tsim)
                bal.append(b)
                bal1.append(b1)
            ...
            if abs(mtspan-ex_tspan) <= tol_tspan and abs(mdist-ex_dist) <= tol_dist and abs(mbal-ex_imb)<=tol_imb and abs(mbal1-ex_imb1)<=tol_imb1 and abs(mbal2-ex_imb2)<=tol_imb2 and abs(mprop-ex_prop)<=tol_prop and abs(mstat-ex_stat)<=tol_stat and abs(mstat2-ex_stat2)<=tol_stat2:
                ...
                for item in range(len(weight_mod1)):
                    par = accept_mod1[:,item]
                    invw += weight_mod1[item]*multivariate_normal.pdf(par, mean=parsim, cov=covmat1)
                weightnewsim = 1/invw
            ...
            while do:
                do = False
                flag = 0
                ind = random.choices(index_mod2, weights=weight_mod2, k=1)[0]
                [birth1sim,birth2sim,death1sim,death2sim,q12sim,q21sim]=accept.T[ind]+multivariate_normal.rvs(np.zeros(6),covmat2)
                w = dom([birth1sim, birth2sim], [death1sim, death2sim], [q12sim,q21sim])  
            ...
            if abs(mtspan-ex_tspan) <= tol_tspan and abs(mdist-ex_dist) <= tol_dist and abs(mbal-ex_imb)<=tol_imb and abs(mbal1-ex_imb1)<=tol_imb1 and abs(mbal2-ex_imb2)<=tol_imb2 and abs(mprop-ex_prop)<=tol_prop and abs(mstat-ex_stat)<=tol_stat and abs(mstat2-ex_stat2)<=tol_stat2:
                ...
                for item in range(len(weight_mod2)):
                    par = accept_mod2[:,item]
                    invw += weight[item]*multivariate_normal.pdf(par, mean=parsim, cov=covmat2)
                weightnewsim = 1/invw
                weightnew.append(weightnewsim)
    ...
    mod2prop.append(mean(acceptmod)) # the proportion of model 2 is selected
...
tol_dist,tol_imb,tol_imb1,tol_imb2,tol_tspan,tol_prop,tol_stat,tol_stat2=generate_tol(accept_mod1, accept_mod2, weight_mod1,weight_mod2, nsim=10, multiplier=40)
...
for k in range(3,T):
    ...
    if accept_rate >= 0.03:
        ...
        tol_dist = math.exp(-0.2*z)*tol_dist
        tol_imb = math.exp(-0.2*z)*tol_imb
        tol_imb1 = math.exp(-0.2*z)*tol_imb1
        tol_imb2 = math.exp(-0.2*z)*tol_imb2
        tol_tspan = math.exp(-0.2*z)*tol_tspan
        tol_prop = math.exp(-0.2*z)*tol_prop
        tol_stat = math.exp(-0.2*z)*tol_stat
        tol_stat2 = math.exp(-0.2*z)*tol_stat2
        accept_rate=0
    ...
    while len(accept1)<M:
        ...
        if indexmod==0:
            ...
            while do:
                ...
                while flag == 0:
                    startphase=1
                    t, flag = birth_death_tree2([birth1sim, birth2sim], [death1sim, death2sim], [q12sim,0], start=startphase, nsize=size)
                    r+=1
                    if r == 5:
                        do = True
                        break
            ...
            if abs(mtspan-ex_tspan) <= tol_tspan and abs(mdist-ex_dist) <= tol_dist and abs(mbala-ex_imb)<=tol_imb and abs(mbala1-ex_imb1)<=tol_imb1 and abs(mbala2-ex_imb2)<=tol_imb2 and abs(mprop-ex_prop)<=tol_prop and abs(mstat-ex_stat)<=tol_stat and abs(mstat2-ex_stat2)<=tol_stat2:
                ...
                for item in range(len(weight_mod2)):
                    par = accept_mod2[:,item]
                    invw += weight_mod2[item]*multivariate_normal.pdf(par, mean=parsim, cov=covmat2)
                weightnewsim = 1/invw
                weightnew.append(weightnewsim)
...
for k in range(T,3*T):
    ...
    if len(index_mod1) == 0:
        ...
        covmat1 = 2*np.cov(accept_mod1, aweights=weight_mod1) # model 2 disappears
    elif len(index_mod1) == 1:
        covmat1 = np.zeros((5,5))
        covmat2 = 2*np.cov(accept_mod2, aweights=weight_mod2)
    elif len(index_mod2) == 1:
        covmat1 = 2*np.cov(accept_mod1, aweights=weight_mod1)
        covmat2 = np.zeros((6,6))
    ...
    acceptmod=[acceptmod[item] for item in accept_index]
