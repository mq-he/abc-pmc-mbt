...
if rank == 0:
    ...
    # no need to record the tolerance level

if rank == 0:
    num_sim = int((50*M)//cpus+1)*cpus # for t=2,3, mean sure it is larger than int(num_sim/cpus)
...
for k in range(1,3):
    ...
    if rank == 0: # compute the parameter values
        ...
        # no need for finding the acceptance rate
    ...
    if rank == 0:
        ...
        dist_output =  compute_dist(output-ex_stats, tol_stats)

        # choose the M closest samples
        # for sorting we using np.argpartition(A, k) which (only) ensures the first kth elements are the k-smallest elements in A
        accept_index = np.argpartition(dist_output, M)[:M]
        assert len(accept_index)==M, f"len(accept_index)={len(accept_index)}, and M={M}"
        # the arrangement in convert2ss matches the original parameter orders
        accept_all = mbt_par_sim[accept_index] # a 2d array, recording all the accepted samples, in the shape of (M,6)
...
if rank == 0:
    num_sim = int((100*M)//cpus+1)*cpus # for mean sure it is larger than int(num_sim/cpus)
else: 
    num_sim = None
...
if rank == 0:
    num_sim = int((100*10*M)//cpus+1)*cpus # for t=2,3, mean sure it is larger than int(num_sim/cpus)
else: 
    num_sim = None
num_sim = comm.bcast(num_sim, root=0)
...
for k in range(T,3*T):
    ...
    if rank == 0:
        ...
        ex_stats = np.array([ex_imb_full, ex_imb1_full, ex_imb2_full, ex_tspan_full, ex_dist_full, ex_prop_full, ex_stat_full, ex_stat2_full])
        tol_stats = np.array([tol_imb, tol_imb1, tol_imb2, tol_tspan, tol_dist, tol_prop, tol_stat, tol_stat2])
        dist_output =  compute_dist(output[:,:8]-ex_stats, tol_stats) # only use the first 8 stats for distance computation

        # choose the 10*M closest samples
        # for sorting we using np.argpartition(A, k) which (only) ensures the first kth elements are the k-smallest elements in A
        M_nLTT = 10*M
        accept_index = np.argpartition(dist_output, M_nLTT)[:M_nLTT]
        assert len(accept_index)==M_nLTT, f"len(accept_index)={len(accept_index)}, and M={M_nLTT}"
        # the arrangement in convert2ss matches the original parameter orders
        accept_all = mbt_par_sim[accept_index] # a 2d array, recording all the accepted samples, in the shape of (M_nLTT,6)
        accept_output = output[accept_index] # a 2d array, recording all 9 stats of the accepted samples, in the shape of (M_nLTT,9)
