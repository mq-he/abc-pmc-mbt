...
# with arbitrary death rates using ABC-PMC with nine selected summary statistics
...
t, flag = birth_death_tree2([birth1,birth2], [death1,death2], [q12,0], nsize=size)
while flag==0:
    t, flag = birth_death_tree2([birth1,birth2], [death1,death2], [q12,0], nsize=size)
...
for i in range(1,iternum):
    flag = 0
    while flag == 0:
        tsim,flag = birth_death_tree2([birth1,birth2], [death1,death2], [q12,0], nsize=size)
...
while len(accept1)<200:
    ...
    while do:
        ...
        while flag == 0 or length != 50:
            t, flag = birth_death_tree2([birth1sim, birth2sim], [death1sim, death2sim], [q12sim,0], nsize=size, max_time=30)
            r+=1
    ...
    for j in range(1,10):
        flag = 0
        while flag == 0:
            tsim, flag = birth_death_tree2([birth1sim, birth2sim], [death1sim, death2sim], [q12sim,0], nsize=size)
        assert flag == 1
...
for k in range(1,T):
    ...
    accept = np.stack((accept1,accept2,accept3,accept4,accept5))
    ...
    while len(accept1)<M:
        ...
        while do:
            ...
            [birth1sim,birth2sim,death1sim,death2sim,q12sim]=accept.T[ind]+multivariate_normal.rvs(np.zeros(5),covmat)
            w = dom([birth1sim, birth2sim], [death1sim, death2sim], [q12sim,0])  
            while w<=0.1 or min(birth1sim, birth2sim,death1sim,death2sim,q12sim)<=0.01 or max(birth1sim, birth2sim,death1sim,death2sim,q12sim)>=5:
                ind = random.choices(index, weights=weight, k=1)[0]
                [birth1sim,birth2sim,death1sim,death2sim,q12sim]=accept.T[ind]+multivariate_normal.rvs(np.zeros(5),covmat)
                w = dom([birth1sim, birth2sim], [death1sim, death2sim], [q12sim,0]) 
            r=0
        ...
        if len(tspann0) > 0:
            ...
            if abs(mtspan-ex_span) <= tol_span and abs(mdist-exdist) <= tol_dist and abs(mbala-ex_imb)<=tol_imb and abs(mbala1-ex_imb1)<=tol_imb1 and abs(mbala2-ex_imb2)<=tol_imb2 and abs(mtsn0-mtn0)<=tol_n0 and abs(meandistp1-mdp1)<=tol_dp1:
                ...
                for item in range(len(weight)):
                    par = np.array([array1[item],array2[item],array3[item],array4[item],array5[item]])
    ...
    meanseq5.append(weighted_mean(accept5, weightnew))
...
for k in range(2*T,3*T):
    ...
    while len(accept1)<M:
        ...
        while do:
            ...
            [birth1sim,birth2sim,death1sim,death2sim,q12sim]=accept.T[ind]+multivariate_normal.rvs(np.zeros(5),covmat)
            w = dom([birth1sim, birth2sim], [death1sim, death2sim], [q12sim,0])  
            while w<=0 or min(birth1sim, birth2sim,death1sim,death2sim,q12sim)<=0.01 or max(birth1sim, birth2sim,death1sim,death2sim,q12sim)>=5:
                ind = random.choices(index, weights=weight, k=1)[0]
                [birth1sim,birth2sim,death1sim,death2sim,q12sim]=accept.T[ind]+multivariate_normal.rvs(np.zeros(5),covmat)
                w = dom([birth1sim, birth2sim], [death1sim, death2sim], [q12sim,0])  
            r=0
